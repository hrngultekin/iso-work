...
class PackageCollectionDialog(QDialog, Ui_PackageCollectionDialog):
    ...
    def fillContent(self):
        ...
        if self.project.selected_languages:
            ...
            for code in self.project.selected_languages:
                self.languagesCombo.addItem(LANGUAGES[code], str(code))
        else:
            self.project.default_language = "en_US"
            self.project.selected_languages.append("en_US")
            self.tmpCollection.translations["en_US"] = ("", "")
            self.languagesCombo.addItem(LANGUAGES["en_US"], str("en_US"))

        if self.tmpCollection.translations and self.tmpCollection.translations[self.project.default_language]:
            self.titleText.setText(str(self.tmpCollection.translations[self.project.default_language][0]))
            self.descriptionText.setPlainText(str(self.tmpCollection.translations[self.project.default_language][1]))
    ...
    def updateTranslations(self, currentIndex):
        code = str(self.languagesCombo.itemData(currentIndex))
        if code and self.tmpCollection.translations[code]:
            self.titleText.setText(str(self.tmpCollection.translations[code][0]))
            self.descriptionText.setPlainText(str(self.tmpCollection.translations[code][1]))
    ...
    def titleChanged(self, text):
        ...
        if code and self.tmpCollection.translations[code]:
            ...
            self.tmpCollection.translations[code] = (str(text), translations[1])
    ...
    def descriptionChanged(self):
        ...
        if code and self.tmpCollection.translations[code]:
            translations = self.tmpCollection.translations[code]
            self.tmpCollection.translations[code] = (translations[0], str(self.descriptionText.toPlainText()))
    ...
    def slotSelectIcon(self):
        iconPath = QFileDialog.getOpenFileName(
            self, _("Select Collection Icon"),
            os.path.join(os.getcwd(), "icons"),
            "*.png")[0]

        if iconPath:
            if self.tmpCollection:
                self.tmpCollection.icon = str(os.path.basename(str(iconPath)))
    ...
    def slotSelectPackages(self):
        if self.tmpCollection.packages.selectedPackages and self.tmpCollection.packages.selectedComponents:
            ...
            if dialog.exec_():
                self.tmpCollection.packages = PackageSet(
                    self.repo_uri,
                    dialog.components,
                    dialog.packages,
                    dialog.all_packages)
